      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8">
        <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-xl shadow-slate-100">
          <div className="grid lg:grid-cols-2">
            <div className="bg-gradient-to-br from-emerald-600 via-emerald-700 to-teal-700 p-8 lg:p-12 flex flex-col justify-between gap-8">
              <div className="space-y-5">
                <div className="inline-flex items-center px-3 py-1 rounded-full bg-white/10 text-emerald-100 text-xs font-semibold border border-white/15">
                  Product Tour
                </div>
                <div>
                  ...
                  <p className="text-sm text-emerald-100 leading-relaxed mt-3 max-w-xs">
                ...
                <ul className="space-y-3">
                  {[
                    ...
                    <li key={item} className="flex items-start gap-2.5">
                      ...
                      <span className="text-sm text-emerald-100 leading-relaxed">{item}</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="flex -space-x-2">
                    {["A", "B", "C", "D"].map((l, i) => (
                      <div key={i} className="w-8 h-8 rounded-full bg-white/20 border-2 border-emerald-600 flex items-center justify-center text-[10px] font-bold text-white">
                        {l}
                      </div>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={label} className="bg-white/8 rounded-lg p-2.5 text-center border border-white/10">
                      <Icon className="w-3.5 h-3.5 text-emerald-200 mx-auto mb-1" />
                      <p className="text-[10px] text-emerald-200 font-medium">{label}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="w-full space-y-5">
                    ...
                    {[
                      ...
                      <div key={field.id} className="space-y-1.5">
                        ...
                        <Input
                          ...
                          className="h-10 rounded-lg border-slate-200 text-sm text-slate-800 placeholder:text-slate-400 focus-visible:ring-emerald-500/30 focus-visible:border-emerald-400"
                        />
                      </div>
                    ))}
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full h-10 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-70 text-white rounded-lg font-semibold text-sm shadow-md shadow-emerald-100 hover:shadow-lg transition-all duration-150 flex items-center justify-center gap-2 group"
                  >
                    {loading ? (
